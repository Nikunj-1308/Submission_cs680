...
public class TimeStampComparatorTest
{
    ...
    @Test
    public void verifyGetChildrenForRepoAsSrcAndTestAndReadme() {
        FSElement[] expected = {readme, src, test};
        Directory directory = repo;
        List<FSElement> element = directory.getChildren((FSElement alphaFS_1, FSElement alphaFS_2) -> alphaFS_1.getName().compareTo(alphaFS_2.getName()));
    ...
    public void verifyGetSubDirectoriesForSrcAsNone() {
        ...
        List<Directory> element = directory.getSubDirectories((FSElement alphaFS_1, FSElement alphaFS_2) -> alphaFS_1.getName().compareTo(alphaFS_2.getName()));
        assertArrayEquals(expected,element.toArray());
    }
    ...
    public void verifyGetFilesForSrcAsAAndB() {
        File[] expected = {a, b};
        Directory directory = src;
        List<File> element = directory.getFiles((FSElement alphaFS_1, FSElement alphaFS_2) -> alphaFS_1.getName().compareTo(alphaFS_2.getName()));
        assertArrayEquals(expected,element.toArray());
    }
}